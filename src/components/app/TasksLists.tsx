...
const TasksLists = ({ task }: TasksListProps) => {
    ...
    return (
        <>
            {openEditDialog && <UpdateDialog task={task} openDialog={openEditDialog} setOpenDialog={setOpenEditDialog} />}
            {openDeleteDialog && <DeleteDialog task={task} openDialog={openDeleteDialog} setOpenDialog={setOpenDeleteDialog} />}
                ...
                <TableCell className="sm:table-cell hidden">
                    <Popover>
                        ...
                        <PopoverContent className="w-32 flex flex-col gap-2 p-4 cursor-pointer">
                            <div className="flex items-center gap-2 font-bold" onClick={() => setOpenEditDialog(true)}>
                                <Edit size={16} />
                                Edit
                            </div>
                            <div className="flex items-center gap-2 text-destructive font-bold" onClick={() => setOpenDeleteDialog(true)}>
